...
const AppProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  // Fetch and keep balance up to date globally
  useEffect(() => {
    if (!loaded) return;
    let active = true;
    const fetchBalance = async () => {
      try {
        const res = await minimaService.getBalance();
        if (active && res) setBalance(res);
      } catch {
        // silently ignore
      }
    };
    ...
    window.addEventListener("minima_balance_update", handleUpdate);
    return () => {
      active = false;
      window.removeEventListener("minima_balance_update", handleUpdate);
    };
  }, [loaded]);
