...
export async function requestProfile(
    ...
    const promise = new Promise<ExtendedProfile>((resolve, reject) => {
        ...
        MDS.cmd.maxima({ params: { action: 'info' } }, (infoRes: any) => {
            ...
            console.log('[ProfileService] Sending profile request via Maxima to:', peerAddress.substring(0, 30));
            ...
            MDS.cmd.maxima({
                ...
                if (!sendRes.status) {
                    ...
                    reject(new Error(`Failed to send profile request: ${sendRes.error}`));
                    return;
                }
