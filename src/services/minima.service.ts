...
SELECT * FROM TRANSACTIONS
    ...
    try {
      ...
      console.error(
        `❌[TX] Failed to find pending transaction by stateId: `,
        err,
      );
      return null;
    }
    ...
    if (
      ...
      try {
        ...
        if (json.type === "mls_register_permanent") {
          console.log(
            "🔭 [DISCOVERY] Registration request received from",
            from,
          );
          ...
          if (pubkey) {
            ...
            MDS.executeRaw(cmd, (res: any) => {
              if (res.status) {
                console.log(
                  "✅ [DISCOVERY] Permanent address added for:",
                  pubkey,
                );
                // Optional: Send confirmation back if needed
              } else {
                console.error(
                  "❌ [DISCOVERY] Failed to add permanent address:",
                  res.error,
                );
              }
            });
            ...
            console.warn(
              "⚠️ [DISCOVERY] Received registration request without public key",
            );
          }
          return;
        }
...
VALUES(UPPER('${peer.pubkey}'), '${escapedAlias}', '${escapedBio}', '${peer.address}', ${now}, 'P2P', ${allowChats})`;
        ...
        if (json.type === "profile_request") {
          console.log(
            "👤 [PROFILE] Request received from",
            from,
            "- Delegating to Service Worker",
          );
...
VALUES('', UPPER('${safeFrom}'), 'System', 'system', 'Maxima contact declined', '', 'sent', 0, ${now}, NULL, ${now})`;
        ...
        if (json.type === "contact_request_received") {
          ...
          this.runSQL(updateSql)
            ...
            .catch((err) => {
              console.error(
                "❌ [CONTACTS] Failed to update message state:",
                err,
              );
            });
          return;
        ...
        console.log(
          "✅ [MAXIMA] Message received (saved by SW):",
          json.message,
        );
      ...
      console.log(
        `ℹ️[MAXIMA] Message ignored(wrong app) "${maximaData.application}"`,
      );
    }
  }
  ...
  async updateMessageState(
    ...
    console.log(
      `🔄[DB] Updating message state: newState = "${state}", timestamp = ${timestamp} `,
    );
...
`;
    ...
    console.log(
      `💾[SQL] Updating message state to '${state}'${newTimestamp ? ` and date to ${newTimestamp}` : ""} for timestamp ${timestamp}`,
    );

    try {
      const result = await this.runSQL(sql);
      console.log(
        `✅[SQL] Message state updated.Result: `,
  ...
  async cleanupStuckMessages(): Promise<void> {
    ...
    console.log(
      `🔍[CLEANUP] Found ${result.rows.length} pending messages.Verifying consistency...`,
    );
    ...
    for (const msg of result.rows) {
      ...
      if (!txResult.rows || txResult.rows.length === 0) {
        ...
        console.log(
          `🗑️[CLEANUP] Message ${timestamp} has NO transaction record - marking as failed`,
        );
        ...
        if (tx.STATUS === "confirmed") {
          ...
          console.log(
            `✅[CLEANUP] Message ${timestamp} has CONFIRMED transaction - fixing state to sent`,
          );
          ...
          console.log(
            `❌[CLEANUP] Message ${timestamp} has REJECTED transaction - fixing state to failed`,
          );
          await this.updateMessageState(msg.PUBLICKEY, timestamp, "failed");
          fixedCount++;
        }
    ...
    if (fixedCount > 0) {
      ...
      console.log(
        `✅[CLEANUP] All pending messages have valid pending transactions`,
      );
    }
  }
  ...
  async getTransactionByMessageTimestamp(
    timestamp: number,
  ): Promise<any | null> {
  ...
  async checkTransactionStatus(txpowid: string): Promise<{
   ...
   */
  async getMyTransactionHistory(): Promise<
    Map<string, { txpowid: string; timestamp: number }>
  > {
    try {
      console.log(
        "🔍 [HISTORY] Fetching transaction history from blockchain...",
      );
      ...
      const historyMap = new Map<
        string,
        { txpowid: string; timestamp: number }
      >();
      const txpows = Array.isArray(txpowResponse.response)
        ? txpowResponse.response
        : [txpowResponse.response];
        ...
        try {
          ...
          if (state && Array.isArray(state) && state.length >= 2) {
            ...
            if (charmChainId === "204") {
              ...
              const timestamp = txpow.header?.timemilli
                ? Number(txpow.header.timemilli)
                : Date.now();

              if (stateId && txpowid) {
                historyMap.set(stateId, { txpowid, timestamp });
                console.log(
                  `✅[HISTORY] Found MetaChain tx: ${stateId} -> ${txpowid} (Time: ${timestamp})`,
                );
              }
            }
          }
      ...
      console.log(
        `✅[HISTORY] Found ${historyMap.size} MetaChain transaction(s) in blockchain`,
      );
      return historyMap;
    } catch (err) {
  ...
  async getMyPendingTransactions(): Promise<Map<string, string>> {
    try {
            ...
            if (state && Array.isArray(state) && state.length >= 2) {
              ...
              if (charmChainId === "204") {
                ...
                if (stateId && txpowid) {
                  ...
                  console.log(
                    `⏳[MEMPOOL] Found pending MetaChain tx: ${stateId} -> ${txpowid} `,
                  );
                }
              }
            }
      ...
      console.log(
        `✅[MEMPOOL] Found ${pendingMap.size} pending MetaChain transaction(s) in mempool`,
      );
      return pendingMap;
    } catch (err) {
   ...
   */
  async checkTransactionByTxpowid(
    txpowid: string,
  ...
  async updateTransactionTxpowid(
    pendinguid: string,
    txpowid: string,
  ): Promise<void> {
  ...
  async updateTransactionStatusByPendingUid(
    pendinguid: string,
    status: "pending" | "sent" | "confirmed" | "rejected",
  ): Promise<void> {
    return transactionService.updateTransactionStatusByPendingUid(
      pendinguid,
      status,
    );
  }
  ...
  async sendCharmWithTokens(
    ...
    console.log(
      `🎯[CHARM] Sending charm ${charmId} with ${amount} Minima to ${recipientName} `,
    );
    ...
    try {
      ...
      const tokenResponse = await this.sendToken(
        "0x00",
        amount.toString(),
        minimaAddress,
        "Minima",
      ...
      if (isTokenPending) {
        console.log(
          `⚠️[CHARM] Token send is pending.Saving message locally but NOT sending via Maxima yet.`,
        );
        ...
        if (txpowid || pendinguid) {
          ...
          console.log(
            `💾[CHARM] Transaction tracked: ${txpowid || "No TXPOWID"} (PendingUID: ${pendinguid || "None"})`,
          );
        } else {
          console.warn(
            `⚠️[CHARM] Could not track transaction: No txpowid AND no pendinguid`,
          );
        }
      ...
      console.log(
        `✅[CHARM] Token sent successfully.Now sending charm message via Maxima...`,
      );
      const msgResponse = await this.sendMessage(
        toPublicKey,
        senderName,
        charmId,
        "charm",
  ...
  async sendToken(
    ...
    try {
      ...
      if (stateId && myPublicKey && recipientPublicKey) {
        ...
        const chatId = await (generateChatId as any)(
          myPublicKey,
          recipientPublicKey,
        );
        ...
        console.log(
          `🏷️[WALLET] Adding state variables: ID = ${stateId}, ChatID = ${chatId}, Sender = ${myPublicKey} `,
        );
      ...
      if (response) {
        ...
        else if (response.response && response.response.txpowid)
          txpowid = response.response.txpowid;
        // 3. Inside txpow object
        else if (
          response.response &&
          response.response.txpow &&
          response.response.txpow.txpowid
        )
          txpowid = response.response.txpow.txpowid;
        // 4. Inside body.txn (common for pending transactions)
        else if (
          response.response &&
          response.response.body &&
          response.response.body.txn &&
          response.response.body.txn.txpowid
        )
          txpowid = response.response.body.txn.txpowid;
      }
      ...
      if (response) {
        ...
        else if (response.response && response.response.pendinguid)
          pendinguid = response.response.pendinguid;
      }
      ...
      if (response && response.status === false) {
        ...
        const isPending =
          response.pending ||
          (response.error &&
            response.error.toString().toLowerCase().includes("pending"));

        if (isPending) {
          console.warn("⚠️ [WALLET] Command is pending approval (Read Mode).");
          console.log(
            "🔍 [DEBUG] Full Pending Response Structure:",
            JSON.stringify(response, null, 2),
          );
          ...
          throw new Error(
            response.error || response.message || "Token send failed",
          );
        }
      }
  ...
  async initProfile() {
    ...
    try {
      ...
      if (cachedAddrRes && cachedAddrRes.status && cachedAddrRes.value) {
        console.log(
          "📍 [PROFILE] Found cached Minima Address in KeyPair:",
          cachedAddrRes.value,
        );
      ...
      if (getAddrRes.status) {
        const myAddress =
          getAddrRes.response.miniaddress || getAddrRes.response.address;
        ...
        if (myAddress) {
          ...
          console.log(
            "📍 [PROFILE] Saved Minima Address to KeyPair for Beacons",
          );
        }
      }
    } catch (err) {
  ...
  init() {
    ...
    console.log(
      "⚙️ [SERVICE] MinimaService initialized - waiting for MDS.init...",
    );
  ...
  private isReconnectEvent(event: any): boolean {
    if (!event) return false;

    if (event.event === "RECONNECTED") return true;
    if (event.data?.type === "RECONNECTED") return true;

    const parseMaybeJson = (value: any): any | null => {
    ...
    const parsedEvent = parseMaybeJson(event.event);
    if (parsedEvent?.type === "RECONNECTED") return true;

    const parsedData = parseMaybeJson(event.data);
    if (parsedData?.type === "RECONNECTED") return true;

    const parsedMessage = parseMaybeJson(event.data?.message);
    if (parsedMessage?.type === "RECONNECTED") return true;
  ...
  processEvent(event: any) {
    ...
    if (this.isReconnectEvent(event)) {
      ...
      chatService.notifyNewMessage({ type: "reconnected" });
      return;
    }
  ...
  async sendSyncStatusCheck(publickey: string) {
    ...
    try {
      const res = await this.runSQL(sql);
      const lastSeq =
        res.rows && res.rows.length > 0 ? res.rows[0].LAST_SEQ || 0 : 0;
      ...
      console.log(
        `🔄 [SMART-SYNC] Sending status check to ${publickey.substring(0, 10)} (Last Seq: ${lastSeq})`,
      );
    ...
    try {
      ...
      if (accept) {
        ...
        if (result && result.status === false) {
          console.log(
            `❌[MDS_PENDING] Transaction ACCEPTED but FAILED execution: ${uid} `,
          );
        ...
        console.log(
          `✅[MDS_PENDING] Transaction ACCEPTED and EXECUTED: ${uid} `,
        );
        ...
        if (result && result.response) {
          ...
          else if (result.response.txpow && result.response.txpow.txpowid)
            txpowid = result.response.txpow.txpowid;
        }
        ...
        const confirmationTime = blockchainTimestamp
          ? Number(blockchainTimestamp)
          : Date.now();
        console.log(
          `🕐[MDS_PENDING] Transaction confirmed at blockchain time: ${confirmationTime} (from header: ${!!blockchainTimestamp})`,
        );
        ...
        if (TYPE === "charm") {
          ...
          console.log(
            `📤[MDS_PENDING] Sending token message via Maxima (Seq: ${seq})...`,
          );
          await this.sendMessage(
            PUBLICKEY,
            username || "Unknown",
        ...
        console.log(
          `✅[MDS_PENDING] Transaction ${uid} processed successfully`,
        );
  ...
  async sendChatRequest(
    ...
    return contactRequestsService.sendChatRequest(
      toAddress,
      myName,
      myAvatar,
      toPublicKey,
  ...
  async acceptChatRequest(
    fromPublicKey: string,
    fromAddress: string,
    options?: { skipMessageInsert?: boolean },
  ): Promise<void> {
    return contactRequestsService.acceptChatRequest(
      fromPublicKey,
      fromAddress,
      options,
    );
  ...
  async resolveMaximaAddress(publicKey: string): Promise<string | null> {
    ...
    try {
      ...
      if (res.rows && res.rows.length > 0) {
        ...
        if (addr && (addr.startsWith("Mx") || addr.startsWith("MX"))) {
          console.log(
            `🔍 [ADDRESS-RESOLVER] Resolved ${publicKey.substring(0, 10)}... to ${addr.substring(0, 10)}...`,
          );
          return addr;
        }
      }
    ...
    console.warn(
      `⚠️ [ADDRESS-RESOLVER] Could not resolve address for ${publicKey.substring(0, 10)}...`,
    );
  ...
  async sendMaximaContactRequest(
    toAddress: string,
    toPublicKey?: string,
  ): Promise<void> {
    return contactRequestsService.sendMaximaContactRequest(
      toAddress,
      toPublicKey,
    );
  }

  async acceptMaximaContactRequest(
    fromPublicKey: string,
    fromAddress: string,
    options?: { skipMessageInsert?: boolean },
  ): Promise<void> {
    return contactRequestsService.acceptMaximaContactRequest(
      fromPublicKey,
      fromAddress,
      options,
    );
  ...
  async declineMaximaContactRequest(
    fromPublicKey: string,
    _fromAddress: string,
    options?: { skipMessageInsert?: boolean },
  ): Promise<void> {
    return contactRequestsService.declineMaximaContactRequest(
      fromPublicKey,
      _fromAddress,
      options,
    );
  ...
  async migrateLegacyChats(): Promise<void> {
    console.log(
      "🧹 [MIGRATION] Checking for legacy chat keys (Mx addresses)...",
    );

    // Find distinct chat keys that look like Maxima Addresses
    const sql =
      "SELECT DISTINCT publickey FROM CHAT_MESSAGES WHERE publickey LIKE 'Mx%'";
    const res = await this.runSQL(sql);
    ...
    console.log(
      `🧹 [MIGRATION] Found ${res.rows.length} legacy chat keys. Attempting to resolve...`,
    );
    ...
    for (const row of res.rows) {
      ...
      if (exactRes.rows && exactRes.rows.length > 0) {
        ...
        console.log(
          `🔄 [MIGRATION] Exact match found! Resolving ${mxAddress} -> ${resolvedPubkey}`,
        );
      } else {
        ...
        if (mxAddress.length > 50) {
          ...
          console.log(
            `🔍 [MIGRATION] Exact match failed. Trying prefix match: ${prefix}...`,
          );
          ...
          if (fuzzyRes.rows && fuzzyRes.rows.length > 0) {
            ...
            console.log(
              `✅ [MIGRATION] Fuzzy match found! Resolving ${mxAddress} -> ${resolvedPubkey}`,
            );
          }
        }
      }
      ...
      if (resolvedPubkey) {
        ...
        console.warn(
          `⚠️ [MIGRATION] Could not resolve ${mxAddress} to a public key (peer not found).`,
        );
      }
    }
  }
  ...
  private async performChatMigration(
    ...
    console.log(
      `🔄 [MIGRATION] Migrating messages from ${oldKey} to ${newKey}...`,
    );
