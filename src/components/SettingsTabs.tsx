...
export function SettingsTabs({ vertical }: SettingsTabsProps) {
  ...
  if (vertical) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700 overflow-hidden w-full max-w-full min-w-0">
        {SETTINGS_TABS.map((tab) => {
          ...
          return (
            <Link
              ...
              to={tab.to}
              disabled={isDisabled}
              className={`flex items-center p-4 transition-colors group min-w-0 max-w-full ${isDisabled ? "opacity-50 pointer-events-none grayscale" : "hover:bg-gray-50 dark:hover:bg-gray-700/50"}`}
            >
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10 transition-colors">
      <div className="max-w-4xl mx-auto min-w-0">
        <nav className="flex justify-start sm:justify-center overflow-x-auto scrollbar-hide -mb-px w-full max-w-full">
          {SETTINGS_TABS.map((tab) => {
            const Icon = tab.icon;
            // Disable if session expired, UNLESS it's Connect or Network
            const isDisabled =
            ...
            return (
              <Link
                ...
                to={tab.to}
                disabled={isDisabled}
                className={`flex items-center justify-center gap-1.5 sm:gap-2 px-4 sm:px-6 py-3 sm:py-4 font-medium text-xs sm:text-sm whitespace-nowrap border-b-2 transition-colors flex-1 sm:flex-none min-w-0 shrink-0 border-transparent text-gray-600 dark:text-gray-400
                                    ${
                                      isDisabled
