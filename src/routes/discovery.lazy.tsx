...
function DiscoveryPage() {
  ...
  const [showRecent, setShowRecent] = useState(true); // Show users offline < 7 days by default
  const [showOld, setShowOld] = useState(false); // Hide users offline > 7 days by default
  ...
  useEffect(() => {
    ...
    const handleDiscoveryUpdate = () => {
      if (DISCOVERY_DEBUG)
        console.log(
          "⚡ [UI] Discovery update event received! Reloading data...",
        );
      loadData();
    };
  ...
  const loadData = async () => {
    ...
    try {
      if (DISCOVERY_DEBUG) console.log("🔄 [DISCOVERY] loadData triggering...");
      ...
      if (DISCOVERY_DEBUG) console.log(`⚡ [DISCOVERY] State updated.`);
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((user) => {
      ...
      const isStale =
        !user.is_online &&
        user.last_updated &&
        Date.now() - Number(user.last_updated) > STALE_THRESHOLD;

      return (
        matchesSearch &&
        matchesCountry &&
        matchesLanguage &&
        (user.is_online || (showRecent && !isStale) || (showOld && isStale))
      );
    });
  }, [users, searchQuery, selectedCountry, selectedLanguage, showRecent, showOld]);
          ...
          <div className="flex flex-wrap gap-3 pt-2 animate-in slide-in-from-top-2 duration-200">
            ...
            <div className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg">
              ...
              <label
                ...
                className="text-sm text-gray-700 dark:text-gray-300 cursor-pointer select-none"
              >
                Older profiles (&gt; 7 days)
              </label>
            </div>

            {(selectedCountry || selectedLanguage || !showRecent || showOld) && (
              <button
                onClick={() => {
                  setSelectedCountry("");
                  setSelectedLanguage("");
          ...
          <>
            <div className="px-6 py-3 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center transition-colors">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                ...
                of <span className="font-medium">{totalFound}</span>{" "}
                {totalFound === 1 ? "profile" : "profiles"}
              </p>
            </div>
