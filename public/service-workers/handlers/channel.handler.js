    ...
    try {
        ...
        channelRunSQL("SELECT channel_id FROM CHANNELS WHERE channel_id = '" + channelId + "'", function (res) {
            if (res.rows && res.rows.length > 0) {
                MDS.log("ℹ️ [CHANNEL] Channel " + channelId + " already exists. Skipping.");
                return;
            }

            // 2. Insert Channel
            var insChannel = "INSERT INTO CHANNELS (channel_id, name, description, admin_publickey, created_date, avatar) " +
            ...
            channelRunSQL(insChannel, function (insRes) {
                if (insRes.status) {
                    ...
                    var insMe = "INSERT INTO CHANNEL_SUBSCRIBERS (channel_id, publickey, username, joined_date, role) " +
                        "VALUES ('" + channelId + "', '" + myPublickey + "', '" + myUsername + "', " + Date.now() + ", 'subscriber')";

                    channelRunSQL(insMe, function (subRes) {
                        if (subRes.status) {
                            MDS.log("✅ [CHANNEL] Successfully joined channel " + channelId);
...
function handleChannelMessage(senderPublickey, maxjson, skipNotify) {
    ...
    MDS.log("📢 [CHANNEL] handleChannelMessage from " + senderPublickey.substring(0, 10));
    ...
    channelRunSQL("SELECT channel_id FROM CHANNELS WHERE channel_id = '" + channelId + "'", function (res) {
        ...
        // 2. Duplicate Check
        var checkSql = "SELECT id FROM CHANNEL_MESSAGES WHERE channel_id='" + channelId + "' AND sender_publickey='" + senderPublickey + "' AND date=" + date;
        MDS.sql(checkSql, function (checkRes) {
            ...
            var counterSql = "SELECT last_seen_seq FROM CHANNEL_MSG_COUNTERS WHERE channel_id='" + channelId + "' AND sender_publickey='" + senderPublickey + "'";

            MDS.sql(counterSql, function (counterRes) {
                var lastSeen = (counterRes.status && counterRes.rows && counterRes.rows.length > 0)
                    ? parseInt(counterRes.rows[0].LAST_SEEN_SEQ)
                    : 0;

                if (senderSeq > 1 && senderSeq > lastSeen + 1) {
                    MDS.log("⚠️ [CHANNEL-GAP] Detected gap for " + channelId + ": last=" + lastSeen + ", new=" + senderSeq + ". Requesting history...");
                    requestChannelHistoryFromSW(channelId);
                }

                // 4. Save Message
                var cmd = "INSERT INTO CHANNEL_MESSAGES (channel_id, sender_publickey, sender_username, type, message, filedata, date, read, sender_seq) " +
                    "VALUES ('" + channelId + "', '" + senderPublickey + "', '" + senderUsername + "', '" + type + "', '" + message + "', '" + filedata + "', " + date + ", 0, " + senderSeq + ")";

                channelRunSQL(cmd, function (insRes) {
                    if (insRes.status) {
                        // 5. Update Counter
                        if (senderSeq > lastSeen) {
                            var upCounterSql = "INSERT INTO CHANNEL_MSG_COUNTERS (channel_id, sender_publickey, last_seen_seq) VALUES ('" + channelId + "', '" + senderPublickey + "', " + senderSeq + ") " +
                                "ON CONFLICT(channel_id, sender_publickey) DO UPDATE SET last_seen_seq = " + senderSeq;
                        ...
                        MDS.log("✅ [CHANNEL] Saved channel message for: " + channelId + " (seq: " + senderSeq + ")");
                    } else {
...
function handleChannelHistoryRequest(pubkey, maxjson) {
    ...
    MDS.log("🔄 [CHANNEL-SYNC] History requested for " + channelId + " since " + since);

    MDS.cmd("maxima action:info", function (info) {
        var myPubkey = info.response.publickey;
        var sql = "SELECT * FROM CHANNEL_MESSAGES WHERE channel_id='" + escapeSql(channelId) + "' AND date > " + since + " ORDER BY date ASC LIMIT 50";
        MDS.sql(sql, function (res) {
            ...
            MDS.cmd("maxima action:send publickey:" + pubkey + " application:metachain-channel data:" + hexData + " poll:false");
        });
    });
}
...
function handleChannelHistoryResponse(pubkey, maxjson) {
    ...
    MDS.log("🔄 [CHANNEL-SYNC] Received " + messages.length + " history messages for " + channelId);
    ...
    // Single notification at the end with a small delay for DB to flush
    setTimeout(function () {
        MDS.comms.solo(JSON.stringify({
            type: "CHANNEL_NEW_MESSAGE",
            channelId: channelId
...
function requestChannelHistoryFromSW(channelId) {
    MDS.log("🔄 [CHANNEL-SYNC] Requesting history for channel " + channelId + "...");

    MDS.cmd("maxima action:info", function (maxInfo) {
        if (!maxInfo.status) return;
        var myPubkey = maxInfo.response.publickey;

        var lastSql = "SELECT date FROM CHANNEL_MESSAGES WHERE channel_id='" + escapeSql(channelId) + "' ORDER BY date DESC LIMIT 1";
        MDS.sql(lastSql, function (lastRes) {
            ...
            var subSql = "SELECT cs.publickey, dp.address FROM CHANNEL_SUBSCRIBERS cs LEFT JOIN DISCOVERED_PEERS dp ON UPPER(cs.publickey)=UPPER(dp.publickey) WHERE cs.channel_id='" + escapeSql(channelId) + "'";
            MDS.sql(subSql, function (subRes) {
                if (!subRes.status || !subRes.rows) return;
                for (var i = 0; i < subRes.rows.length; i++) {
                    ...
                    var cmd = (cleanAddr && (cleanAddr.startsWith("Mx") || cleanAddr.startsWith("MX")))
                        ? "maxima action:send to:" + cleanAddr + " application:metachain-channel data:" + hexData + " poll:false"
                        : "maxima action:send publickey:" + subPk + " application:metachain-channel data:" + hexData + " poll:false";
                    MDS.cmd(cmd);
                }
            });
        });
...
function requestAllChannelsHistory() {
    ...
    MDS.sql("SELECT channel_id FROM CHANNELS", function (res) {
        if (!res.status || !res.rows || res.rows.length === 0) return;
        for (var i = 0; i < res.rows.length; i++) {
            requestChannelHistoryFromSW(res.rows[i].CHANNEL_ID || res.rows[i].channel_id);
        }
    });
}
...
function handleChannelInfoUpdate(pubkey, maxjson) {
    MDS.log("📢 [CHANNEL] handleChannelInfoUpdate from " + pubkey.substring(0, 10));

    var channelId = maxjson.channelId;
    if (!channelId) return;

    var updates = [];
    if (maxjson.newName) updates.push("name='" + maxjson.newName.replace(/'/g, "''") + "'");
    if (maxjson.newDescription !== undefined) updates.push("description='" + (maxjson.newDescription || "").replace(/'/g, "''") + "'");
    if (maxjson.avatar !== undefined) updates.push("avatar='" + (maxjson.avatar || "").replace(/'/g, "''") + "'");

    if (updates.length === 0) return;

    var cmd = "UPDATE CHANNELS SET " + updates.join(", ") + " WHERE channel_id='" + channelId + "'";
...
function handleChannelRoleUpdate(pubkey, maxjson) {
    ...
    MDS.sql(checkSenderSql, function (resSender) {
        if (!resSender.status || !resSender.rows || resSender.rows.length === 0) return;
...
function handleChannelJoinRequest(pubkey, maxjson) {
    try {
        var channelId = maxjson.channelId;
        var requesterName = (maxjson.requesterName || "Anonymous").replace(/'/g, "''");
        var requesterAddress = (maxjson.requesterAddress || pubkey).replace(/'/g, "''");

        MDS.log("🎟️ [CHANNEL] Join request for " + channelId + " from " + requesterName);
        ...
        MDS.cmd("maxima action:info", function (info) {
            ...
            // 1. Get channel info
            MDS.sql("SELECT * FROM CHANNELS WHERE channel_id='" + channelId + "'", function (chanRes) {
                if (!chanRes.rows || chanRes.rows.length === 0) return;
                var chan = chanRes.rows[0];

                // 2. Add subscriber
                var insSub = "INSERT INTO CHANNEL_SUBSCRIBERS (channel_id, publickey, username, joined_date, role) " +
                    "VALUES ('" + channelId + "', '" + pubkey + "', '" + requesterName + "', " + Date.now() + ", 'subscriber')";
                ...
                MDS.sql(insSub, function (insRes) {
                    ...
                    MDS.cmd("maxima action:send to:" + requesterAddress + " application:metachain-channel data:" + hexData + " poll:false");

                    // 4. Send SYSTEM MESSAGE locally
                    var systemMsg = requesterName + " joined the channel";
                    var insSys = "INSERT INTO CHANNEL_MESSAGES (channel_id, sender_publickey, sender_username, type, message, date, read) " +
                        "VALUES ('" + channelId + "', 'system', 'system', 'system', '" + systemMsg + "', " + Date.now() + ", 0)";
                    MDS.sql(insSys);

                    MDS.comms.solo(JSON.stringify({ type: "CHANNEL_NEW_MESSAGE", channelId: channelId }));
                });
            });
        });
    } catch (err) { }
