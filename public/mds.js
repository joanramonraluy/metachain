...
var MDS = {
	...
	init: function (callback) {
		...
		//CAPACITOR APK DETECTION: If running at localhost or port 0,
		//we're inside the Capacitor APK and need to connect to the local Minima node
		if (host === "localhost" || port === 0 || host === "127.0.0.1") {
			MDS.log("🚀 CAPACITOR APK DETECTED - Configuring for local Minima node");
			host = "127.0.0.1";
			port = 9003;
			if (MDS.minidappuid == null) {
				MDS.minidappuid = "0x00"; // Fallback if not in URL
			}
		}
		...
		// ALWAYS force http for local node connection to avoid SSL handshake issues in WebView
		if (host === "127.0.0.1" || host === "localhost") {
			protocol = "http:";
		}

		// HARD SET if debug protocol found
		if (MDS.DEBUG_PROTOCOL != null) {
			protocol = MDS.DEBUG_PROTOCOL;
		}

		MDS.log("DEBUG: host=" + host + " protocol=" + protocol);
			...
			if (status === 0 || (status >= 200 && status < 400)) {
				...
				if (callback) {
					try {
						...
						MDS.log("Error parsing JSON: " + e);
						// Return a failure object to avoid hangs
						callback({ status: false, error: "Invalid JSON response" });
					}
				}
			...
			if (status === 0 || (status >= 200 && status < 400)) {
				...
				if (callback) {
					try {
						...
						MDS.log("Error parsing Poll JSON: " + e);
					}
				}
